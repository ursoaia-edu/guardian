...
try {
    ...
    $possiblePaths = @(
        (Join-Path $projectRoot "bin\agent\procsentinel-agent64.exe"),
        (Join-Path $projectRoot "agent\procsentinel-agent64.exe"),
        (Join-Path (Split-Path $projectRoot -Parent) "agent\procsentinel-agent64.exe")
    )
    ...
    if ($service) {
        ...
        $oldExePath = Join-Path $InstallPath "procsentinel-agent64.exe"
...
"@
    ...
    $installedExePath = Join-Path $InstallPath "procsentinel-agent64.exe"
