...
Write-Host "  Guardian 32bit Uninstaller" -ForegroundColor Cyan
Write-Host "=====================================" -ForegroundColor Cyan
Write-Host ""

try {
    $exePath = Join-Path $InstallPath "procsentinel-agent32.exe"
    ...
    if (-not $service) {
        Write-Host "      Guardin not found" -ForegroundColor Yellow
        if (-not $RemoveFiles) {
            Write-Host ""
        ...
        Write-Host "      Service found: Guardian" -ForegroundColor Gray
        ...
        if ($service.Status -eq "Running") {
            ...
            if ($service -and $service.Status -eq "Running") {
                if ($Force) {
                    Write-Host "      WARNING: Service still running, proceeding anyway..." -ForegroundColor Yellow
                } else {
        ...
        # Remove service
        Write-Host ""
        Write-Host "[2/3] Removing Guardian..." -ForegroundColor Green
    ...
    if ($service) {
        ...
        Write-Host "=====================================" -ForegroundColor Cyan
        Write-Host ""
        Write-Host "[OK] Guardian uninstalled successfully" -ForegroundColor Green
    }
